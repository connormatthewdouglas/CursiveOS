...
- **G3 — Autonomous proposer. MVP BUILT (2026-07-02).** `tools/organism_proposer.py` selects the next candidate from an audited library of reversible `sysctl` knobs (priority-ordered, memory-channel-first since that is where the last win came from), materializes a real parent-delegating + reversible preset and variant file, and prints a privileged enqueue SQL. Safety: audited knobs only (never free-form shell), reversible by construction, `simulated_not_payout_eligible`/Linux-scoped, and enqueue stays privileged (respects the queue RLS — anon cannot inject). First autonomous candidate materialized: `v0.13-pagecluster0` (`vm.page-cluster=0`, the zram companion v0.12 lacks). See `docs/os0-organism-proposer.md`. **Cycle 5 (2026-07-06): the full autonomous loop closed** — privileged enqueue → daemon claim on Stardust → screen → upload → trust-spine rows → request/job `complete`, with zero manual screen steps. Candidate read **neutral on every weighted channel** (memory refault −0.9% ≈ noise): honest null, hypothesis not supported on Stardust. The screen also surfaced the **evidence-gate/CPU-bound collision** and the **rig config-drift bug — both fixed 2026-07-06** (commit `d69587a`: honest hardware flags void only their channel; `config_version` auto-heals stale rig configs). A laptop screen through the fixed gate confirmed the machinery (`inconclusive` 0.50, fitness −0.0023, sustained voided) and the null: **pagecluster0 is neutral on both machines — retired; next knob is `vfscache50`.** **Next:** ground selection in the live QD archive (`tools/qd_organism.py`) from real CursiveRoot fitness, and add gated auto-enqueue via a signed proposer identity (needs G4) so the loop closes without the manual SQL step — real reward still hard-gated.
- **G4 — Trust / independence layer.** Productize the V local trust model: immutable raw-artifact recomputation, signed machine/session identity, global accepted-fingerprint replay index, CursiveRoot-owned independent aggregation, hardware/wallet independence, and immune/anomaly sensors. **Known alpha residual (hardened where cheap 2026-07-02):** `measurement_requests` is now privileged-authored only (anon INSERT dropped) and anon may update only `(status, updated_at)` — queue injection and execution-redirect are closed, verified against the live anon key. But anon can still cross-write rows in `machine_capabilities` and `measurement_jobs` (`USING(true)` update policies) because without authenticated identity a self-asserted `machine_id` cannot be trusted. Closing this fully = the signed-identity-gated write path (part of G4); it carries no money (`payout_eligible` hard-false) and no code-execution redirection, but must land before external-tester rollout. V rejects fabricated evidence, global replays, parsimony overclaims, and funded confirmation-Sybil derivation patterns. **Sprint 3 shipped the first CursiveRoot DB trust spine** (`os0_identity_keys`, `os0_raw_artifact_index`, `os0_trust_evaluations`) while keeping payout eligibility hard-disabled. Hardware/wallet independence and production aggregation remain the hard gate in front of money.
...
- **Done by 2026-07-06:** the `claude/*` branches on CursiveResearch are gone from origin — merged via `0d47ba1` ("research: merge pending corpus branches") and pruned.
- **Done 2026-07-06 (3 of 4):** red-team flags resolved in `CursiveResearch/VALIDATION.md` — Mesa 260% and AlphaEvolve 23% narrowed via Ch14/Ch20 living-layer do-not-cite rows; Ch15 truncation confirmed present in the source DOCX itself (import is faithful). **Open:** the BBR single-flow/default-preset flag — gated on the Ch09 multi-flow fairness/retransmit experiment (Gap #3/#5) plus a human decision on default-preset/public copy.
...
- **CPU inference correctly suppressed** — cold-start is the honest CPU story; sustained CPU inference delta is unreliable due to thermal variance from C-state changes. **Root cause (2026-07-06):** stock ollama ships CUDA/ROCm/Metal backends only — no Intel Arc (SYCL/oneAPI) support — so the A750 sits idle during inference and sustained falls back to CPU. Since 2026-07-06 the sensor voids (not fraud-rejects) the sustained channel on such hosts (`voided_channels`). **Laptop FIXED same day:** the GTX 1650 was on nouveau; installing the Canonical-signed nvidia-580 driver (Secure Boot intact) took tinyllama from 33.4 → ~166 tok/s (5.0×, 100% GPU; phi3 ~25 tok/s at 81% offload). Laptop sustained is a real channel again; pre-driver laptop numbers are CPU-era (not comparable across 2026-07-06). **Stardust unlock BUILT (2026-07-06 evening):** ipex-llm SYCL ollama installed as an isolated second instance (`~/ollama-arc`, port 11435) + Intel compute runtime; verified `23/23 layers offloaded` to the A750 (SYCL0 buffers). Clean performance number and harness integration pending an idle window — until integrated, Stardust's sustained channel stays voided.
