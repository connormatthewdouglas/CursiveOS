...
- **G3 — Autonomous proposer. MVP BUILT (2026-07-02).** `tools/organism_proposer.py` selects the next candidate from an audited library of reversible `sysctl` knobs (priority-ordered, memory-channel-first since that is where the last win came from), materializes a real parent-delegating + reversible preset and variant file, and prints a privileged enqueue SQL. Safety: audited knobs only (never free-form shell), reversible by construction, `simulated_not_payout_eligible`/Linux-scoped, and enqueue stays privileged (respects the queue RLS — anon cannot inject). First autonomous candidate materialized: `v0.13-pagecluster0` (`vm.page-cluster=0`, the zram companion v0.12 lacks). See `docs/os0-organism-proposer.md`. **Cycle 5 (2026-07-06): the full autonomous loop closed** — privileged enqueue → daemon claim on Stardust → screen → upload → trust-spine rows → request/job `complete`, with zero manual screen steps. Candidate read **neutral on every weighted channel** (memory refault −0.9% ≈ noise): honest null, hypothesis not supported on Stardust. The screen also surfaced the **evidence-gate/CPU-bound collision** (see HANDOVER — the hardened gate fraud-rejects honest founder-rig runs because sustained inference is always CPU-bound there; must be fixed before any further screens can produce decision-grade verdicts). **Next:** fix that gate scoping, then ground selection in the live QD archive (`tools/qd_organism.py`) from real CursiveRoot fitness, and add gated auto-enqueue via a signed proposer identity (needs G4) so the loop closes without the manual SQL step — real reward still hard-gated.
