...
- **CPU inference correctly suppressed** — cold-start is the honest CPU story; sustained CPU inference delta is unreliable due to thermal variance from C-state changes. **Root cause (2026-07-06):** stock ollama ships CUDA/ROCm/Metal backends only — no Intel Arc (SYCL/oneAPI) support — so the A750 and the laptop's GTX 1650 (as installed) sit idle during inference and sustained falls back to CPU on both founder rigs. Since 2026-07-06 the sensor voids (not fraud-rejects) the sustained channel on such hosts (`voided_channels`). Unlock path: ipex-llm ollama build or llama.cpp SYCL — would make sustained a real selection channel on founder hardware.
