...
### Platform scope (2026-09-14)

Local models on phones and tablets are a real workload class. They are **not** the same substrate as Stardust.

Policy: [`docs/specs/platform-substrates.md`](docs/specs/platform-substrates.md). Research: [`docs/research/mobile-local-compute-2026-09-14.md`](docs/research/mobile-local-compute-2026-09-14.md).

- One organism, several substrates. One ISO for PC and phone is forbidden.
- Selection-truth remains `linux-x86_64-workstation` / parent v0.12.
- Mobile rows, if they ever exist, are observe-only until a mobile parent is declared. They cannot move v0.12.
- Transition 1 stays the amd64 remix ISO. Mobile does not block it.
- iOS is not an OS target.
...
Platform families (desktop vs android-qcom vs linux-arm64-phone) are a second axis, specified in `docs/specs/platform-substrates.md`. Do not collapse them into one parent.
...
First installable CursiveOS. Tuned distribution, benchmarks included, contributor/measurement daemon running. No natural-language shell yet -- this release validates the ISO build path and the daemon on real user hardware. (Release numbers are independent of the preset lineage's v0.9 -- see the naming note in Transition 1.) amd64 first. Not a phone image.
