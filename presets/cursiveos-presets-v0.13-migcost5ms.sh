#!/usr/bin/env bash
# CursiveOS v0.13-migcost5ms candidate — AUTONOMOUSLY PROPOSED by organism_proposer.
#
# = the v0.12 parent stack PLUS one reversible sysctl knob: kernel.sched_migration_cost_ns=5000000.
# Primary sensor channel: sustained.
#
# Hypothesis (pre-registered): Raising sched_migration_cost_ns (0.5ms->5ms) makes the scheduler less eager to migrate warm inference threads across cores, which can help sustained tok/s and cold-start. Single-stream sustained is near its noise floor, so treat a small move as inconclusive.
#
...
STATE="$HOME/CursiveOS/preset_state_v0.13-migcost5ms.txt"
...
echo "CursiveOS Candidate v0.13-migcost5ms (v0.12 stack + $KEY=$VAL)"
...
case "$ACTION" in
  ...
  --dry-run)
    bash "$PARENT" --dry-run
    echo "  + sysctl: $KEY=$VAL (channel sustained; prior value captured for undo)"
    ;;
  --apply-temp)
    ...
    echo "OK Applied v0.13-migcost5ms temporarily."
    ;;
  --undo)
    if [[ -f "$STATE" ]]; then
    ...
    bash "$PARENT" --undo
    echo "OK v0.13-migcost5ms reverted (sysctl + v0.12 stack)."
    ;;
