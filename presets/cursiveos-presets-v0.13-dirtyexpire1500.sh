#!/usr/bin/env bash
# CursiveOS v0.13-dirtyexpire1500 candidate — AUTONOMOUSLY PROPOSED by organism_proposer.
#
# = the v0.12 parent stack PLUS one reversible sysctl knob: vm.dirty_expire_centisecs=1500.
# Primary sensor channel: memory.
#
# Hypothesis (pre-registered): Expiring dirty pages sooner (3000->1500 cs) smooths writeback bursts that can interfere with reclaim under pressure. Expect memory channel neutral-to-better, sustained neutral.
#
...
STATE="$HOME/CursiveOS/preset_state_v0.13-dirtyexpire1500.txt"
...
echo "CursiveOS Candidate v0.13-dirtyexpire1500 (v0.12 stack + $KEY=$VAL)"
...
case "$ACTION" in
  ...
  --apply-temp)
    ...
    echo "OK Applied v0.13-dirtyexpire1500 temporarily."
    ;;
  --undo)
    if [[ -f "$STATE" ]]; then
    ...
    bash "$PARENT" --undo
    echo "OK v0.13-dirtyexpire1500 reverted (sysctl + v0.12 stack)."
    ;;
