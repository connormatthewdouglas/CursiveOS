...
- **Next:** (1) clean Arc benchmark on idle Stardust (`~/ollama-arc/start-arc.sh`, then `OLLAMA_HOST=127.0.0.1:11435 ollama run …`) — expect a laptop-like multiple over the 6.7 tok/s CPU mistral; (2) decide integration: point the harness at 11435 via env override vs replacing system ollama with the SYCL build (then Stardust's sustained channel un-voids); (3) pull Stardust's clone (still pre-fix at `7901ee6`) before its next screen — config auto-heals on first run; (4) enqueue `v0.13-vfscache50`; (5) QD-grounded proposer + gated auto-enqueue (G4); (6) wire nvidia-smi GPU power into the laptop harness; (7) hardware/wallet independence hardening before any real BTC/reward path.
