...
- **Next:** (1) pull Stardust's clone (still pre-fix at `7901ee6`) before its next screen — config will auto-heal on first run; (2) enqueue `v0.13-vfscache50` (proposer materialize + privileged SQL) on an idle machine; (3) ground proposer selection in the live QD archive + gated auto-enqueue via signed proposer identity (G4); (4) Intel Arc inference backend experiment (ipex-llm / llama.cpp SYCL) on Stardust — the laptop's 5× shows what it's worth; (5) wire nvidia-smi GPU power into the harness on the laptop; (6) hardware/wallet independence hardening before any real BTC/reward path.
