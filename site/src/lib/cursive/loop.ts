...
export function loopLabel(state: LoopState) {
  if (state === "running") return "measuring";
  if (state === "queued") return "work waiting";
  if (state === "stale") return "waiting for a machine";
  if (state === "blocked") return "held for safety";
  return "resting";
}

export function organismBlurb(state: LoopState) {
  if (state === "running") return "A machine is running a test right now.";
  if (state === "queued") return "There is work waiting for a machine to pick it up.";
  if (state === "stale") return "No machine has checked in today. The organism is waiting.";
  if (state === "blocked") return "Results exist, but we will not treat them as settled until more than one machine agrees.";
  return "Awake, but not testing anything new. That is an honest rest, not a failure.";
}
