...
# Progress goes to stderr; only the median triplet is captured per state.
...
echo "Power probe  N=$N settle=${SETTLE}s  cpu_e=$([[ -f $CPU_E ]] && echo yes||echo no) gpu_e=${GPU_E:-none} gt=${GPU_GT:-none}" >&2
[[ -z "$GPU_GT" ]] && { echo "No Arc GT path; this probe targets Intel Arc. Aborting."; exit 1; }

read_pair() {  # prints "cpu_w gpu_w" for one 1s window
...
sample_state() {  # $1 label -> stdout: "cpu_med gpu_med total_med n"; progress -> stderr
...
restore_gpu; sleep 3
A=$(sample_state "A UNPINNED (v0.9 default)")
...
python3 -c "print(f'GPU pin cost: +{$bg-$ag:.2f}W gpu, +{$bt-$at:.2f}W total -> this is what v0.9 SAVES by dropping the pin')"
